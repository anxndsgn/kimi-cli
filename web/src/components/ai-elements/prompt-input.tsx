				...
				<div className="w-auto space-y-3">
					<div className="flex items-center gap-2.5">
						<div className="min-w-0 flex-1 space-y-1 px-0.5">
							<h4 className="truncate font-semibold text-sm leading-none">{attachmentLabel}</h4>
							{data.mediaType && (
...
export function PromptInputAttachments({
	...
	return (
		<div className={cn("flex flex-wrap items-start gap-2 p-3 w-full", className)} {...props}>
...
export const PromptInput = ({
	...
	const validateMediaFile = useCallback((file: File): { valid: boolean; error?: string } => {
		const isImage = file.type.startsWith("image/");
		const isVideo = file.type.startsWith("video/");

		if (isImage) {
			if (
				!IMAGE_CONFIG.allowedTypes.includes(file.type as (typeof IMAGE_CONFIG.allowedTypes)[number])
			) {
				return {
					valid: false,
			...
			if (
				!VIDEO_CONFIG.allowedTypes.includes(file.type as (typeof VIDEO_CONFIG.allowedTypes)[number])
			) {
				return {
					valid: false,

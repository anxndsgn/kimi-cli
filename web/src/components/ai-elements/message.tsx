...
export const MessageContent = ({ children, className, ...props }: MessageContentProps) => (
	<div className={cn("flex w-full flex-col gap-1 overflow-hidden text-sm", className)} {...props}>
		{children}
	</div>
);
...
export const UserMessageContent = ({ children, className, ...props }: UserMessageContentProps) => {
	return (
		<div
			className={cn(
...
export const MessageCopyButton = ({ content, timeout = 2000 }: MessageCopyButtonProps) => {
		...
		<AlertDialog>
			...
			<AlertDialogContent size="sm">
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						A new session will be created with the conversation history up to and including this
						response. The current session will not be affected.
					</AlertDialogDescription>
				</AlertDialogHeader>
				<AlertDialogFooter>
...
export function MessageAttachment({ data, className, onRemove, ...props }: MessageAttachmentProps) {
	...
	): attachment is NoPreviewAttachment => "kind" in attachment && attachment.kind === "nopreview";
				...
				<Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
					...
					>
						<DialogHeader className={isText ? "p-4 pb-0" : "sr-only"}>
							<DialogTitle>{isText ? filename : "Attachment preview"}</DialogTitle>
						</DialogHeader>
